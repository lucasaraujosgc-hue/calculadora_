...
export default function App() {
  ...
  useEffect(() => {
    if (formData.dataAdmissao && formData.dataDemissao) {
      ...
      while (current <= endDate) {
        dates.push({
            date: current.toISOString().slice(0, 7),
            value: 0
        });
  ...
  const handleCalcular = () => {
    ...
    setCalculo({
        ...
        valorAviso: valorAvisoProvento, valorAvisoDesconto, diasAviso, projecaoAviso, 
        valor13, avos13, valorFeriasVencidas, tercoFeriasVencidas, valorFeriasProp, tercoFeriasProp, avosFerias: avosFeriasCalc,
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                ...
                <div className="mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-2 bg-slate-100 p-1.5 rounded-xl">
                        <button type="button" onClick={() => setFormData(prev => ({ ...prev, motivo: 'dispensa' }))} className={`py-2 px-3 rounded-lg text-sm font-medium transition-all ${formData.motivo === 'dispensa' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Sem Justa Causa</button>
                        <button type="button" onClick={() => setFormData(prev => ({ ...prev, motivo: 'pedido' }))} className={`py-2 px-3 rounded-lg text-sm font-medium transition-all ${formData.motivo === 'pedido' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Pedido de Demissão</button>
                    </div>
                </div>
                ...
                {formData.motivo === 'pedido' && formData.avisoTipo === 'indenizado' && (
                    <div className="bg-orange-50 text-orange-700 text-xs p-3 rounded-lg mb-4 border border-orange-100"><strong>Nota:</strong> Como é pedido de demissão indenizado, o valor será descontado (30 dias).</div>
                )}
                ...
                <div className="space-y-6 animate-fade-in">
                    ...
                    <Card title="FGTS + Multa 40%" icon="savings" delay="delay-300" onClick={() => setShowFGTSModal(true)} action={<span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded uppercase">Editar</span>}>
                        {calculo.isPedidoDemissao ? (
                            <div className="text-sm text-slate-500 italic py-2">Pedido de Demissão: Sem multa de 40% e sem saque imediato. <br/><span className="text-xs text-slate-400">(Apenas depósito do mês devido)</span></div>
                        ) : (
                            <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Card title="Proventos" icon="add_circle_outline" delay="delay-400" action={<button onClick={() => setShowAdjustModal(true)} className="text-xs font-bold text-indigo-600 hover:text-indigo-800">AJUSTAR</button>}>
                            ...
                            {calculo.valorFeriasDobro > 0 && (<><ResultRow label="Férias em Dobro" value={calculo.valorFeriasDobro} subtext={`${calculo.qtdFeriasDobro} período(s)`} /><ResultRow label="1/3 s/ Férias em Dobro" value={calculo.tercoFeriasDobro} /></>)}
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center pt-8 no-print">
                         <button onClick={() => setShowAdjustModal(true)} className="bg-white text-indigo-700 border border-indigo-200 hover:bg-indigo-50 px-6 py-3 rounded-xl shadow-sm flex items-center gap-2 transition-all font-semibold"><span className="material-icons-round">post_add</span> Adicionar Provento/Desconto</button>
                        <button onClick={() => setShowPrintModal(true)} className="bg-slate-800 hover:bg-slate-900 text-white px-8 py-3 rounded-xl shadow-lg flex items-center gap-2 transition-all font-semibold"><span className="material-icons-round">print</span> Imprimir Relatório</button>
                    </div>
                </div>
            )}
          </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="p-6 overflow-y-auto">
                    <div className="mb-6 bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                        ...
                        <input type="number" className="w-full px-4 py-2 border border-indigo-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Informe o saldo total acumulado" value={fgtsSaldoManual} onChange={(e) => setFgtsSaldoManual(Number(e.target.value))} />
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {fgtsManualData.map((item, idx) => (
                            <div key={idx}><label className="block text-xs font-medium text-slate-500 mb-1">{item.date}</label><input type="number" className="w-full px-3 py-1.5 border rounded text-sm" value={item.value} onChange={(e) => updateFgtsValue(idx, Number(e.target.value))} /></div>
                        ))}
                    </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm no-print">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                ...
                <form onSubmit={addAjuste} className="p-6 space-y-4">
                    <div><label className="block text-sm font-bold text-slate-700 mb-1">Descrição</label><input name="descAjuste" required className="w-full border p-3 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
                    <div><label className="block text-sm font-bold text-slate-700 mb-1">Valor (R$)</label><input name="valAjuste" type="number" step="0.01" required className="w-full border p-3 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
                    <div><label className="block text-sm font-bold text-slate-700 mb-1">Tipo</label><select name="tipoAjuste" className="w-full border p-3 rounded-lg focus:ring-2 focus:ring-indigo-500 bg-white"><option value="Provento">Provento (+)</option><option value="Desconto">Desconto (-)</option></select></div>
                    <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-lg font-bold mt-4 shadow-lg">Adicionar</button>
                </form>
                    ...
                    <ul className="space-y-2 max-h-40 overflow-y-auto pr-1">
                        {ajustes.map((aj, i) => (
                            <li key={i} className="flex justify-between items-center text-sm bg-slate-50 p-3 rounded border border-slate-100">
                                ...
                                <span className={`font-bold font-mono ${aj.tipo === 'Provento' ? 'text-green-600' : 'text-red-600'}`}>{aj.tipo === 'Provento' ? '+' : '-'} {formatCurrency(aj.valor)}</span>
                            </li>
                        ))}
                    </ul>
                    <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
                        <button onClick={() => { handleCalcular(); setShowAdjustModal(false); }} className="px-6 py-3 bg-slate-800 text-white rounded-lg hover:bg-slate-900 font-bold w-full shadow-md">Concluir</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div className="bg-slate-200 min-h-screen w-full flex flex-col items-center py-8 print:bg-white print:p-0 print:h-full">
                ...
                <div className="bg-white p-4 rounded-xl shadow-lg mb-8 w-full max-w-4xl no-print">
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 flex flex-col gap-3">
                         <label className="flex items-center gap-2 cursor-pointer select-none w-fit"><input type="checkbox" checked={printSignatures} onChange={e => setPrintSignatures(e.target.checked)} className="rounded text-indigo-600 focus:ring-indigo-500" /><span className="text-sm font-medium text-slate-700">Incluir campos de assinatura</span></label>
                         {printSignatures && (<div className="animate-fade-in"><label className="block text-xs font-bold text-slate-500 mb-1 uppercase">Texto sobre a assinatura</label><textarea value={signatureText} onChange={(e) => setSignatureText(e.target.value)} rows={3} className="w-full text-sm p-2 border border-slate-300 rounded focus:ring-2 focus:ring-indigo-500 outline-none" /></div>)}
                    </div>
                </div>
                ...
                <div id="print-area-container" className="bg-white w-full h-full p-8 shadow-none mx-auto relative text-sm text-slate-900 flex flex-col justify-between print:p-8">
                    <div className="print-content-wrapper">
                        <div className="border-b-2 border-slate-900 pb-4 mb-6 flex justify-between items-start">
                            <div><h1 className="text-xl font-bold uppercase tracking-wider">Demonstrativo de Valores</h1><p className="text-xs font-semibold text-slate-500 uppercase mt-1">Cálculo Rescisório Trabalhista ({calculo.isPedidoDemissao ? 'Pedido de Demissão' : 'Dispensa sem Justa Causa'})</p></div>
                            <div className="text-right"><div className="text-[10px] text-slate-500 uppercase">Data</div><div className="font-mono font-bold">{formatDate(new Date())}</div></div>
                        </div>
                        ...
                        <table className="w-full text-sm text-left mb-6 border-collapse">
                            <thead className="bg-slate-800 text-white text-xs uppercase"><tr><th className="p-2 border border-slate-800">Rubrica</th><th className="p-2 border border-slate-800 text-center w-24">Ref.</th><th className="p-2 border border-slate-800 text-right w-32">Proventos</th><th className="p-2 border border-slate-800 text-right w-32">Descontos</th></tr></thead>
                            <tbody className="text-xs">
                                <tr className="border-b border-slate-200"><td className="p-2">Saldo de Salário</td><td className="p-2 text-center text-slate-500">{calculo.diasTrabalhados}d</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.saldoSalario)}</td><td className="p-2 text-right font-mono"></td></tr>
                                {calculo.valorAviso > 0 && <tr className="border-b border-slate-200 bg-slate-50"><td className="p-2">Aviso Prévio Indenizado</td><td className="p-2 text-center text-slate-500">{calculo.diasAviso}d</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.valorAviso)}</td><td className="p-2 text-right font-mono"></td></tr>}
                                <tr className="border-b border-slate-200"><td className="p-2">13º Salário Proporcional</td><td className="p-2 text-center text-slate-500">{calculo.avos13}/12</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.valor13)}</td><td className="p-2 text-right font-mono"></td></tr>
                                {calculo.valor13Indenizado > 0 && <tr className="border-b border-slate-200 bg-slate-50"><td className="p-2">13º Salário s/ Aviso</td><td className="p-2 text-center text-slate-500">-</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.valor13Indenizado)}</td><td className="p-2 text-right font-mono"></td></tr>}
                                {calculo.valorFeriasVencidas > 0 && <tr className="border-b border-slate-200"><td className="p-2">Férias Vencidas</td><td className="p-2 text-center text-slate-500">-</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.valorFeriasVencidas)}</td><td className="p-2 text-right font-mono"></td></tr>}
                                {calculo.tercoFeriasVencidas > 0 && <tr className="border-b border-slate-200 bg-slate-50"><td className="p-2">1/3 Férias Vencidas</td><td className="p-2 text-center text-slate-500">1/3</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.tercoFeriasVencidas)}</td><td className="p-2 text-right font-mono"></td></tr>}
                                {calculo.valorFeriasDobro > 0 && <tr className="border-b border-slate-200"><td className="p-2">Férias em Dobro</td><td className="p-2 text-center text-slate-500">{calculo.qtdFeriasDobro}</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.valorFeriasDobro)}</td><td className="p-2 text-right font-mono"></td></tr>}
                                {calculo.tercoFeriasDobro > 0 && <tr className="border-b border-slate-200 bg-slate-50"><td className="p-2">1/3 s/ Férias em Dobro</td><td className="p-2 text-center text-slate-500">1/3</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.tercoFeriasDobro)}</td><td className="p-2 text-right font-mono"></td></tr>}
                                <tr className="border-b border-slate-200"><td className="p-2">Férias Proporcionais</td><td className="p-2 text-center text-slate-500">{calculo.avosFerias}/12</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.valorFeriasProp)}</td><td className="p-2 text-right font-mono"></td></tr>
                                <tr className="border-b border-slate-200 bg-slate-50"><td className="p-2">1/3 Férias Proporcionais</td><td className="p-2 text-center text-slate-500">1/3</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.tercoFeriasProp)}</td><td className="p-2 text-right font-mono"></td></tr>
                                {calculo.valorFeriasIndenizado > 0 && <tr className="border-b border-slate-200"><td className="p-2">Férias s/ Aviso</td><td className="p-2 text-center text-slate-500">-</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.valorFeriasIndenizado)}</td><td className="p-2 text-right font-mono"></td></tr>}
                                {calculo.tercoFeriasIndenizado > 0 && <tr className="border-b border-slate-200 bg-slate-50"><td className="p-2">1/3 s/ Férias Indenizadas</td><td className="p-2 text-center text-slate-500">1/3</td><td className="p-2 text-right font-mono">{formatCurrency(calculo.tercoFeriasIndenizado)}</td><td className="p-2 text-right font-mono"></td></tr>}
                                {ajustes.filter(a => a.tipo === 'Provento').map((aj, idx) => (<tr key={`prov-${idx}`} className="border-b border-slate-200"><td className="p-2 text-indigo-700">{aj.descricao}</td><td className="p-2 text-center text-slate-500">Manual</td><td className="p-2 text-right font-mono">{formatCurrency(aj.valor)}</td><td className="p-2 text-right font-mono"></td></tr>))}
                                <tr className="border-b border-slate-200 text-red-700"><td className="p-2">INSS</td><td className="p-2 text-center text-slate-500">Desc.</td><td className="p-2 text-right font-mono"></td><td className="p-2 text-right font-mono">{formatCurrency(calculo.descontoINSS)}</td></tr>
                                {calculo.totalIRRF > 0 && <tr className="border-b border-slate-200 text-red-700"><td className="p-2">IRRF (Tabela 2026)</td><td className="p-2 text-center text-slate-500">Desc.</td><td className="p-2 text-right font-mono"></td><td className="p-2 text-right font-mono">{formatCurrency(calculo.totalIRRF)}</td></tr>}
                                {calculo.valorAvisoDesconto > 0 && <tr className="border-b border-slate-200 text-red-700"><td className="p-2">Aviso Prévio (Não Trabalhado)</td><td className="p-2 text-center text-slate-500">30d</td><td className="p-2 text-right font-mono"></td><td className="p-2 text-right font-mono">{formatCurrency(calculo.valorAvisoDesconto)}</td></tr>}
                                {ajustes.filter(a => a.tipo === 'Desconto').map((aj, idx) => (<tr key={`desc-${idx}`} className="border-b border-slate-200 text-red-700"><td className="p-2">{aj.descricao}</td><td className="p-2 text-center text-slate-500">Manual</td><td className="p-2 text-right font-mono"></td><td className="p-2 text-right font-mono">{formatCurrency(aj.valor)}</td></tr>))}
                            </tbody>
                            <tfoot className="bg-slate-100 font-bold border-t-2 border-slate-300">
                                <tr><td className="p-3" colSpan={2}>TOTAIS</td><td className="p-3 text-right text-slate-800">{formatCurrency((calculo.rescisaoLiquida + calculo.descontoINSS + calculo.totalIRRF + calculo.valorAvisoDesconto + ajustes.filter((a: any) => a.tipo === 'Desconto').reduce((acc: number, c: any) => acc + c.valor, 0)))}</td><td className="p-3 text-right text-red-600">{formatCurrency(calculo.descontoINSS + calculo.totalIRRF + calculo.valorAvisoDesconto + ajustes.filter((a: any) => a.tipo === 'Desconto').reduce((acc: number, c: any) => acc + c.valor, 0))}</td></tr>
                            </tfoot>
                        </table>
                        ...
                        <div className="mb-8">
                            <div className="flex justify-end items-center mb-2 px-4 gap-4"><span className="text-sm font-bold text-slate-500 uppercase">Rescisão Líquida a Receber</span><span className="text-lg font-mono font-bold text-slate-700">{formatCurrency(calculo.rescisaoLiquida)}</span></div>
                            <div className="border-2 border-slate-800 p-4 flex justify-between items-center bg-slate-50"><div><div className="text-xs font-bold uppercase text-slate-500">Total Geral a Receber</div><div className="text-[10px] text-slate-400">Rescisão Líquida {calculo.isPedidoDemissao ? '' : '+ Total FGTS'}</div></div><div className="text-3xl font-bold text-slate-900 font-mono">{formatCurrency(calculo.totalGeral)}</div></div>
                        </div>
                    </div>
                    ...
                    <div className="mt-auto">
                        ...
                        <div className="pt-8 mb-8">
                            {signatureText && (<p className="text-xs text-justify text-slate-600 mb-6 px-4 leading-relaxed whitespace-pre-line">{signatureText}</p>)}
                            <div className="grid grid-cols-2 gap-12 border-t border-slate-200 pt-8">
                                <div className="text-center"><div className="h-10"></div><div className="border-t border-slate-400 pt-2 font-bold text-xs uppercase">Assinatura do Empregador</div></div>
                                <div className="text-center"><div className="h-10"></div><div className="border-t border-slate-400 pt-2 font-bold text-xs uppercase">Assinatura do Empregado</div></div>
                            </div>
                        </div>
                        )}
                        <div className="border-t border-slate-200 pt-4 flex items-center gap-4">
                            ...
                            <div><div className="text-lg font-black uppercase text-slate-800 tracking-wide">Lucas Araujo dos Santos</div><div className="text-sm font-semibold text-slate-500 uppercase">Contador • CRC-BA: 046968/O-6</div></div>
                        </div>
                    </div>
                </div>
            </div>
